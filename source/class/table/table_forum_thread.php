...
class table_forum_thread extends discuz_table
{
	...
	public function search_condition($conditions, $prefix = false) {
		...
		if(isset($conditions['sticky'])) {
	        ...
	        if(isset($conditions['usedtime'])) {
	            if($conditions['usedtime'] == 1) {
	                $wherearr[] = $prefix.DB::field('lastpost', strtotime(date('Y-m-d',strtotime('-1 day'))), '>');
	                $this->_urlparam[] = "usedtime=1";
	            } elseif($conditions['usedtime'] == 2) {
	                $wherearr[] = $prefix.DB::field('lastpost', strtotime(date('Y-m-d',strtotime('-3 days'))), '>');
	                $this->_urlparam[] = "usedtime=2";
	            } elseif($conditions['usedtime'] == 3) {
	                $wherearr[] = $prefix.DB::field('lastpost', strtotime(date('Y-m-d',strtotime('-1 week'))), '>');
	                $this->_urlparam[] = "usedtime=3";
	            } elseif($conditions['usedtime'] == 4) {
	                $wherearr[] = $prefix . DB::field('lastpost', strtotime(date('Y-m-d',strtotime('-3 week'))), '>');
	                $this->_urlparam[] = "usedtime=4";
	            }else{
	                $wherearr[] = $prefix . DB::field('lastpost', strtotime(date('Y-m-d',strtotime('last month'))), '>');
	                $this->_urlparam[] = "usedtime=5";
	            }
	        }
