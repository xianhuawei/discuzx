...
class table_forum_forum extends discuz_table
{
	...
	public function fetch_info_by_fid($fid) {
		//加缓存
		$mem_key = 'forum_forum_fetch_info_by_fid_'.$fid;
		$result = memory('get',$mem_key);
		if($result == false){
	...
	public function fetch_all_info_by_fids($fids, $status = 0, $limit = 0, $fup = 0, $displayorder = 0, $onlyforum = 0, $noredirect = 0, $type = '', $start = 0) {
		//如果只是主键查询则缓存
		$pr_key = !empty($fids) && !in_array($fids) && (count($fids) == 1 ) && empty($fup) && !empty($onlyforum) && empty($type) && empty($noredirect) && empty($displayorder) && empty($limitsql) && empty($ordersql);
		$mem_key = 'forum_forum_fetch_all_info_by_fids_'.$fid;
		$mem_result = false;
		if($pr_key){
			$mem_result = memory('get',$mem_key);
			if($mem_result != false){
				return $mem_result;
			}
		}
		...
		$result = DB::fetch_all("SELECT ff.*, f.* FROM %t f LEFT JOIN %t ff USING (fid) WHERE $sql $ordersql $limitsql", array($this->_table, 'forum_forumfield'), 'fid');
		if($pr_key ){
			memory('set',$mem_key,$result,600);
		}
		return $result;
	...
	public function fetch_todayposts() {
		...
		memory('set',$mem_key,$todayposts,60);//缓存60秒
