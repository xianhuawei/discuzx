...
class block_actors {
	...
	function blockclass() {
		return array('actors', lang('block/hskvcenter', 'hsk_actors'));
	}
	...
	function getsetting() {
		...
		$settings = array(
			'aids' => array(
				'title' => lang('block/hskvcenter', 'hsk_actorid'),
				'type' => 'text',
				'default' => '0'
			),
			'sids' => array(
	...
	function getdata($style, $parameter) {
		...
		$aids		= intval($parameter['aids']) ? intval($parameter['aids']) : 0;
		...
		$query = DB::query("SELECT t.itemid as id, v.vsubject, v.purl, v.vsum, v.abtotal, v.uid, v.views, v.polls, v.valuate, v.remote, v.vinfo, m.username FROM ".DB::table('vgallery_tags')." t LEFT JOIN ".DB::table('vgallerys')." v ON v.id=t.itemid LEFT JOIN ".DB::table('common_member')." m ON m.uid=v.uid where t.tagid='$aids' and t.idtype='HSKACT' and v.audit=1 and v.subid=0 $sql ORDER BY v.$orders DESC limit $limits");
