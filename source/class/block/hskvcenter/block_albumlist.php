...
class block_albumlist {
	...
	function blockclass() {
		return array('albumlist', lang('block/hskvcenter', 'hsk_album_list'));
	}
	...
	function getsetting() {
		...
		$settings = array(
			...
			'albums' => array(
				...
				'value' => array(
					array('0', lang('block/hskvcenter', 'hsk_vod_all')),
					array('1', lang('block/hskvcenter', 'hsk_userlist')),
					array('2', lang('block/hskvcenter', 'hsk_desklist')),
				)
			),
			'limits' => array(
	...
	function getstylelist(){
		...
		$newarray = array();
		//生成数组
		foreach($xml_list['albumlist'] as $key=>$val){
	...
	function getdata($style, $parameter) {
		...
		$sql = 	($sids ? " AND v.fid='$sids'" : null)
			.$price_sql
			.($albums ? " AND v.album='$albums'" : " AND v.album in(1,2)");

		$query = DB::query("SELECT v.id, v.vsubject, v.purl, v.vsum, v.abtotal, v.uid, v.views, v.polls, v.valuate, v.remote, v.vinfo, m.username FROM ".DB::table('vgallerys')." v LEFT JOIN ".DB::table('common_member')." m ON m.uid=v.uid where v.audit=1 $sql ORDER BY v.$orders DESC limit $limits");
