...
class model_forum_post extends discuz_model {
	...
	protected function _init_parameters($parameters) {
		$varname = array(
            ...
            'model','rom_version','product_version',
            'contact','probability','description','steps','snapshot','logurl',
            'funtions','backgrounds'
		);
	...
	public function newreply($parameters) {
		...
		if($this->param['modnewreplies']) {
			...
			//下沉贴 不更新这个
			if($this->thread['lastpost'] < getglobal('timestamp') && $this->thread['lastpost'] > getglobal('timestamp') - 63072000) {
	...
	public function deletepost($parameters) {
		...
		if($isfirstpost) {
			...
			//下沉的 不更新lastpost
			if($lastpost['dateline'] > getglobal('timestamp') - 63072000 ){
