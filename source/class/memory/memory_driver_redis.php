...
class memory_driver_redis
{
	...
	function init($config) {
		//如果多个redis,随机选择一个
		$redis_server_count = count($config['proxy']);
		if(!empty($config['server']) || ($redis_server_count>0 && !empty($config['proxy'][$redis_server_count-1]['server'])) ) {
			if(!empty($config['proxy'][$redis_server_count-1]['server'])){
				$redis_server_count = $redis_server_count - 1;
				$redis_id = intval(rand(0,$redis_server_count));
				$config = $config['proxy'][$redis_id];
			}
			try {
				$this->obj = new Redis();
				if($config['pconnect']) {
			...
			if($this->enable) {
				if( isset($config['requirepass']) && !empty($config['requirepass']) ) {
