...
var rowtypedata = [
	...
	function addjscall() {
		...
		if(blockclass==1 || blockclass==2 || blockclass==8 || blockclass==9) {
			showWindow('getvalue', 'plugin.php?id=hsk_vcenter:hsk_vcenter&mod=getlist&types=indexset&ccc=1&styleid=' + blockclass);
		} else if(blockclass) {
			$('valuenew').value = '0';
			alert('<?=cplang("hsk_indexset_info")?>');
			$('valuenew').select();
		} else {
...
if(!$cp) {
	...
	if(!submitcheck('actorsubmit')) {
		...
		$navlist = array();
		$query = DB::query("SELECT * FROM ".DB::table('vgallery_indexset')." WHERE typeid='1' order by dsp, id");
	...
	if(!submitcheck('editsubmit')) {
		...
		DB::query("UPDATE ".DB::table('vgallery_indexset')." SET subject='$subjectnew', styleid='$stylenew', moremsg='$moremsgnew', value='$valuenew', getsum='$getsumnew' WHERE id='$id'");
