...
if(!$cp) {
	...
	if(!submitcheck('actorsubmit')) {
		...
		$navlist = array();
		$query = DB::query("SELECT * FROM ".DB::table('vgallery_ad')." WHERE styleid in(41,42) order by id");
	...
	if(!submitcheck('editsubmit')) {
		...
		if($editnext){//自动跳转下一个类别
			$nav = DB::fetch_first("SELECT id FROM ".DB::table('vgallery_ad')." WHERE id>'$id' and styleid in(41,42)");
			if(!$nav) {
