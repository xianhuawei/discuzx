...
if($cp == "edit" && $id) {//修改分类
	...
	$count = DB::result_first("SELECT COUNT(*) FROM ".DB::table('vgallery_actor')." WHERE director='1'".$sqladd);
	$multipage = multi($count, $ppp, $page, ADMINSCRIPT."?action=hsk&operation=types&do=$do".$filteradd);
	
	
	$query = DB::query("SELECT * FROM ".DB::table('vgallery_actor')." where director='1' $sqladd LIMIT $start_limit, $ppp");
