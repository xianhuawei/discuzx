		...
		if(!submitcheck('submit')) {
			...
			foreach($navlist as $nav) {
				...
				if(!empty($subnavlist[$nav['sid']])) {
					showtagheader('tbody', 'subnav_'.$nav['sid'], false);
...
EOT;
		...
		if(!submitcheck('editsubmit')) {
			...
			//浏览权限
			$groupselect = array();
			$bygroupvar = unserialize($nav['sygroup']);
			...
			while($group = DB::fetch($query)) {
				...
				$groupselect[$group['type']] .= '<option value="'.$group['groupid'].'"'.(@in_array($group['groupid'], $bygroupvar) ? ' selected' : '').'>'.$group['grouptitle'].'</option>';
			}
			...
			$usergroupselect = '<select name="sygroupnew[]" size="10" multiple="multiple">'.$groupselect.'</select>';

			//发布权限
			$bygroupvar = unserialize($nav['regroup']);
			$query = DB::query("SELECT type, groupid, grouptitle, radminid FROM ".DB::table('common_usergroup')." WHERE groupid NOT IN ('6', '7') ORDER BY (creditshigher<>'0' || creditslower<>'0'), creditslower, groupid");
			while($regroup = DB::fetch($query)) {
				$regroup['type'] = $regroup['type'] == 'special' && $regroup['radminid'] ? 'specialadmin' : $regroup['type'];
				$regroupselect[$regroup['type']] .= '<option value="'.$regroup['groupid'].'"'.(@in_array($regroup['groupid'], $bygroupvar) ? ' selected' : '').'>'.$regroup['grouptitle'].'</option>';
			}

			$regroupselect .= '<optgroup label="'.$lang['usergroups_member'].'">'.$regroupselect['member'].'</optgroup>'.
				($regroupselect['special'] ? '<optgroup label="'.$lang['usergroups_special'].'">'.$regroupselect['special'].'</optgroup>' : '').
				($regroupselect['specialadmin'] ? '<optgroup label="'.$lang['usergroups_specialadmin'].'">'.$regroupselect['specialadmin'].'</optgroup>' : '').
				'<optgroup label="'.$lang['usergroups_system'].'">'.$regroupselect['system'].'</optgroup></select>';

			$reusergroupselect = '<select name="sygroupnew[]" size="10" multiple="multiple">'.$groupselect.'</select>';

			

			if(!$nav['sup']) {
				//绑定论坛
				$fupselect = "<select name=\"bandnew\">\n<option value=\"0\">".cplang('hsk_band_no')."</a>\n";
				...
				if(is_array($fups)) {
					foreach($fups as $forum1) {
						if($forum1['type'] == 'group') {
							$fup_group[$forum1['fid']] .= "$forum1[name]";
						}else{
							$selected = $forum1['fid'] == $nav['band'] ? "selected=\"selected\"" : NULL;
							$fup_select[$forum1['fup']] .= "<option value=\"$forum1[fid]\" $selected>&nbsp; &gt; $forum1[name]</option>\n";
						}
					}
				}
				foreach($fup_group as $key=>$val){
					$fupselect .= "<optgroup label=\"$val\">$val</optgroup>".$fup_select[$key];
				}
				$fupselect .= '</select>';
			}elseif($nav['supband']){
				//绑定主题分类
			...
			DB::query("UPDATE ".DB::table('vgallery_sort')." SET sort='$namenew', sup='$supnew', scolor='$colornew', band='$bandnew', sortman='$sortmannew', sortmoney='$sortmoneynew', sygroup='$sygroupnew', regroup='$regroupnew' WHERE sid='$id'");
