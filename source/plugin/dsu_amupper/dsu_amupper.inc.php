...
if($this_time <> 0){
    ...
    if($tsarr && $thisvars['ms'] == 3){
        ...
        foreach ($data_f2a as $id => $result){
            if(($_G['groupid'] == $result['usergid']|| $result['usergid'] <= '0') && $cons == $result['days']){
                $teshu[$id] = $result;
                $tsmsg[] = array('title'=>$_G['setting']['extcredits'][$result['extcredits']]['title'], 'reward'=>$result['reward']);
            }
        }
    }
    //有特殊奖励（循环）
    ...
    if(file_exists(DISCUZ_ROOT.'./data/tid_amupper.lock')) {
        exit(json_encode(array('status'=>0,'message'=>'非法提交数据','checkin'=>0)));
    }else{
        $jiangliba = 0;
...
function isH($time){
    ...
    if($pperconfig['max'] && $pperconfig['minreward'] && $pperconfig['maxreward']){
        ...
        $nt = $nt1*60 + $nt2;
        $Htime =$_G['timestamp'] - $nt;
        $Hnum = C::t('plugin_dsuamupper')->count_by_lasttime($Htime);
        $time = empty($time) ? 0 : $time ;
