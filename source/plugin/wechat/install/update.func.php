		...
		foreach($newtables as $i => $newtable) {
			...
			if(!$query = DB::query("SHOW CREATE TABLE ".DB::table($newtable), 'SILENT')) {
				...
				if($maths[3] == 'MEMORY' || $maths[3] == 'HEAP') {
					$type = helper_dbtool::dbversion() > '4.1' ? " ENGINE=MEMORY".(empty($config['dbcharset'])?'':" DEFAULT CHARSET=$config[dbcharset]" ): " ENGINE=HEAP";
				} else {
					$type = helper_dbtool::dbversion() > '4.1' ? " ENGINE=InnoDB".(empty($config['dbcharset'])?'':" DEFAULT CHARSET=$config[dbcharset]" ): " ENGINE=InnoDB";
				}
				$usql = $maths[1].$type;
