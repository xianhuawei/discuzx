...
//权限限制
$thread = DB::fetch_first("SELECT authorid,userfield,setting FROM ".DB::table('forum_thread')." A,".DB::table('plugin_xj_event')." B WHERE A.tid='$tid' and A.tid = B.tid");
...
if($_GET['action']=='verify' and $_GET['applyid']){
	...
	if($_GET['verifyaction']==1){
		...
		if($items['use_extcredits_num']>0){
			$query = DB::query("SELECT uid FROM ".DB::table('plugin_xj_eventapply')." WHERE applyid in($applyids)");
			while($value = DB::fetch($query)){
				updatemembercount($value['uid'],array($items['use_extcredits']=>$items['use_extcredits_num']));
			}
		}
		DB::query("DELETE FROM ".DB::table('plugin_xj_eventapply')." WHERE applyid in($applyids)");
	}
}
...
$listcount = DB::result_first("SELECT COUNT(*) FROM ".DB::table('plugin_xj_eventapply')." WHERE tid='$tid'");
$perpage = 10; //每页数
...
$query = DB::query("SELECT * FROM ".DB::table('plugin_xj_eventapply')." A,".DB::table('common_member')." B WHERE A.uid = B.uid and A.tid = '$tid' ORDER BY A.verify,A.dateline DESC LIMIT $start_limit,$perpage");
$joinlist = array();
