...
if($_GET['pluginop'] == 'add' && submitcheck('adduser')) {
	if($singleprem && in_array($_GET['usernamenew'], $permusers) || !$singleprem) {
		...
		if(C::t('#myrepeats#myrepeats')->count_by_uid_username($_G['uid'], $usernamenew)) {
			DB::query("UPDATE ".DB::table('plugin_myrepeats')." SET logindata='$logindata' WHERE uid='$_G[uid]' AND username='$usernamenew'");
		} else {
			$_GET['commentnew'] = addslashes($_GET['commentnew']);
			DB::query("INSERT INTO ".DB::table('plugin_myrepeats')." (uid, username, logindata, comment) VALUES ('$_G[uid]', '$usernamenew', '$logindata', '".strip_tags($_GET['commentnew'])."')");
		}
		dsetcookie('mrn', '');
		dsetcookie('mrd', '');
