	...
	function getguisepostuserid($guisepost_type='post'){
		...
		if($myguiseuid){
			...
			if($this->vars['showmod']){
				include template('guisepost:input');
				return $return;
			}else{
...
//论坛使用
class plugin_guisepost_forum extends plugin_guisepost{
	//提交成功后处理后续事件
