...
function TAR_get($link, $host) {
	...
	//標題
	preg_match_all("/title\" content=\"(.*?)\">/i", $c, $datarow);
	//轉碼
	$TAR_return['subject'] = $datarow[1][0];
	...
	//視頻地址
	preg_match_all("/v\=([\w\-]+)/", $link, $datarow);
	   if(!empty($datarow[1][0])) {
		   $TAR_return['flashid'] = $datarow[1][0];
           $TAR_return['flashurl'] = "http://www.youtube.com/v/".$datarow[1][0]."?version=3&amp;hl=zh_TW";
		}
	//以上，視頻地址結束。
