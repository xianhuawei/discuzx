...
if(file_exists(DISCUZ_ROOT.'./data/plugindata/bank_ane.lang.php')){
	include DISCUZ_ROOT.'./data/plugindata/bank_ane.lang.php';
} else {
	loadcache('pluginlanguage_template');
	loadcache('pluginlanguage_script');
	$scriptlang['bank_ane'] = $_G['cache']['pluginlanguage_script']['bank_ane'];
	$templatelang['bank_ane'] = $_G['cache']['pluginlanguage_template']['bank_ane'];
}
...
if(!is_array($hackSettings)) {
	showmessage($bankmsglang['config_error']);
}
$hackVersion = '<a href="http://addon.discuz.com/?@682.developer">2.2</a>';
$hackName = $navigation = $hackSettings['name'];
$hackCopyright = $hackSettings['copyright'] ? $hackSettings['copyright'] : 'Designed by Wz2006.Com';
...
if(!isset($extcredits[$moneycreditsnum]['title'])) {
	...
	showmessage($bankmsglang['configexc_error']);
}
...
if($hackVars['adminisbankadm']==1 && $_G[adminid]==1) $issupbankadmin=1;
