...
function deletethread($tids, $membercount = false, $credit = false, $ponly = false) {
	...
	// 收集待删的tid、fid、posttableid、threadtables
	...
	C::t('forum_typeoptionvar')->delete_by_tid($arrtids);// 从上面移动到这里处理
	C::t('forum_poststick')->delete_by_tid($arrtids);// 从上面移动到这里处理
...
function deleteblogs($blogids, $force = false) {
	...
	foreach($query as $value) {
		if($allowmanage || $value['uid'] == $_G['uid']) {
			...
			//积分
			if($value['status'] == 0) {// 如果没有通过审核就删除，则不需要处理积分和用户日志数
				if($value['uid'] != $_G['uid']) {
