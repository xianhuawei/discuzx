 ...
 * @param obj
 */
function articFun(obj){
	$j(obj).each(function(i,v){
		if(i>2){
			return false;
		}
		var cls = "";
...
function signinFunc(btn,tipBox){
	$j(btn).on("click",function(){
		var _url = $j(this).attr("attribute");
		var _tipCont = $j(tipBox).find(".tipcont_signin");
		var _btnCont = $j(btn).find(".btn_sign_left");
		var _signin = $j(this).hasClass("btn_signed")?1:0;
		if(_signin == 1){return false;}//若已签到，跳出方法
		if(_url.match("login") != null){//若为登录链接，给出登录提示
			clearTimeout(counter);
			$j(tipBox).html("请先<a href='member.php?mod=logging&amp;action=login' target='_blank'>登录</a>");
			$j(tipBox).show();
			...
			$j.ajax({
				...
				success:function(data){
					...
					if(status == 1 && checkin == 1 && _signin == 0 && signcount>0){
						$j(btn).addClass("btn_signed");
						_btnCont.find('p').text("已签到");
						$j('#today_count_sign').text(signcount);
					}
					if(message){
						clearTimeout(counter);
