...
function editorsimple() {
	...
	var b2rs = btn.getElementsByTagName("div");
	if($(editorid + '_body').className == 'edt') {
		v = 'none';
		$(editorid + '_simple').innerHTML = '高级模式';
		$(editorid + '_body').className = 'edt simpleedt';
		$(editorid + '_adv_s1').className = 'b2r';
		$(editorid + '_adv_s1').style.width = "136px";
		$(editorid + '_adv_s1').getElementsByTagName("p")[0].style['clear'] = "none";
		...
		simplodemode = 1;
		for(var k=0; k<b2rs.length; k++){
			var origClass = b2rs[k].className;
			b2rs[k].className = origClass + " simpleedtStyle";
			if(k===0){
				b2rs[k].style.borderRight = "none";
			}
		}
	} else {
		v = '';
		$(editorid + '_simple').innerHTML = '普通模式';
		...
		simplodemode = 0;
		for(var k=0; k<b2rs.length; k++){
			var newClass = b2rs[k].className.split(" simpleedtStyle")[0];
			b2rs[k].className = newClass;
			if(k===0){
				b2rs[k].style.borderRight = "1px solid #ddd";
			}
		}
	}
	...
	var btns = $(editorid + '_adv_s1').getElementsByTagName('A');
	for(i = 0;i < btns.length;i++) {
		if(btns[i].getAttribute('adv')) {
			btns[i].style.display = v;
		}
	}
	var btns = $(editorid + '_adv_s2').getElementsByTagName('A');
	var br = $(editorid + '_adv_s2').getElementsByTagName('br');
	if(br[0].getAttribute('adv')) {
		br[0].style.display = v;
	}
	for(i = 0;i < btns.length;i++) {
		if(btns[i].getAttribute('adv')) {
			btns[i].style.display = v;
...
function insertAllImg(){
    var obj = jQuery('#imgattachlist').find("td[class!='imgdeleted']");
    jQuery(obj).each(function(i,v){
        if(jQuery(v).attr("id")){
            var reg = /id="image_td_((\w|-|\s)+)/ig;
            id =  jQuery(v).attr("id").replace("image_td_",'');
            insertAttachimgTag(id);
            doane();
        }
    });
}
...
function insertAttachimgTag(aid) {
	if(wysiwyg) {
        if(editdoc.body.innerHTML.indexOf('attachimg_' + aid)<0){
            insertText('<img src="' + $('image_' + aid).src + '" border="0" aid="attachimg_' + aid + '" alt="" />&emsp;', false);
        }
	} else {
		...
		switch(tag) {
			...
			default:
				...
				if(!str) {
                    ...
                    if(tag=="qq" && !custombbcodes[tag]){
                        custombbcodes[tag] = {"prompt":'请输入 QQ 号码:<a href="" class="xi2" onclick="this.href=\'http://wp.qq.com/set.html?from=discuz&uin=\'+$(\'e_cst1_qq_param_1\').value" target="_blank" style="float:right;">设置QQ在线状态&nbsp;&nbsp;</a>'};
                    }
