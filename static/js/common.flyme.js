...
function showMenu(v) {
	...
	var is_nav = false;
	if(v['ctrlclass'] == 'hover' && v['duration'] == 2 ) is_nav = true;
	...
	if(pos != '*') {
		setMenuPosition(showid, menuid, pos,is_nav);
	}
...
function setMenuPosition(showid, menuid, pos,is_nav) {
	var showObj = $(showid);
	...
	if(direction == 0 || menuObj.scrolly) {
		if(BROWSER.ie && BROWSER.ie < 7) {
			...
			menuObj.style.position = mobileplayer() ? 'absolute' : 'fixed';
		}
	}
	//如果是导航 则left减去40 居中
	//if(is_nav) ml = ml - 30;
...
function showWindow(k, url, mode, cache, menuv, callback) {
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (!!("placeholder" in document.createElement("input"))) {
		if(searchtxt.value == '请输入您要搜索的帖子') {
			searchtxt.value = '';
		}
		searchtxt.placeholder = '请输入您要搜索的帖子';
	} else {
        searchtxt.value = '请输入您要搜索的帖子';
...
function strLen(obj, checklen, maxlen) {
    var v = obj.value, charlen = 0, maxlen = !maxlen ? 200 : maxlen, curlen = maxlen, len = strlen(v);
    for(var i = 0; i < v.length; i++) {
            curlen -= charset == 1;
    }
    if(curlen >= len) {
        ...
        obj.value = mb_cutstrUTF8(v, maxlen, '');
    }
}

function mb_cutstrUTF8(str, maxlen, dot) {
    var len = 0;
    var ret = '';
    //var dot = !dot ? '...' : dot;
    maxlen = maxlen - dot.length;
...
function noticeTitle() {
	...
	//}
    $j(".J_msg_tips").css("visibility","visible");
    $j("#msgLen").html(tipNum);
