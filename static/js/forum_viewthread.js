...
function supportupdate(n,type) {
    if (getcookie('support')) {
        setTimeout(function () {
            $('occur_num').innerHTML =parseInt($('occur_num').innerHTML)+n;
...
function postreviewupdate(pid, n) {
    ...
    var content = objv.innerHTML.replace("(", "").replace(")", "");
    var num = parseInt(!!content ? content : 0) + 1;
    objv.innerHTML = "(" + num + ")";
}
