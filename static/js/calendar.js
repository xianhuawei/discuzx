...
function settime(d) {
	...
	if(istippredate){
		var v = yy + "-" + zerofill(mm + 1) + "-" + zerofill(d) + (addtime ? ' ' + zerofill($('hour').value) + ':' + zerofill($((halfhour) ? 'minutehalfhourly' : 'minute').value) : '');
		var vFormat = parsedate(v);
		var now = new Date();
		if(vFormat>now){
			controlid.value = v;
		}else{
			showError("选择时间必大于当前时间");
		}
	}
	else{
...
function showcalendar(event, controlid1, addtime1, startdate1, enddate1, halfhour1, recall) {
	...
	istippredate = (controlid.id=="cronpublishdate") ? true : false;
	addtime = addtime1;
